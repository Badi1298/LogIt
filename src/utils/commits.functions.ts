...
export const analyzeWeeklyCommits = createServerFn({ method: "POST" })
	...
	.handler(async ({ data: commits }) => {
        ...
        4. For each ticket, calculate the total sum of all the hours. Then, provide a breakdown of work done for each day in the \`dailyWork\` array. For each day, include the date (e.g., 'Mon 26th'), the time spent that day, and a list of specific achievements for that day.
